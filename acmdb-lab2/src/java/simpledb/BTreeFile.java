...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		if (e == null)
			throw new DbException("what the fuck with findLeafPage?");
		BTreePageId nxt;
		if (left) nxt = e.getLeftChild();
		else nxt = e.getRightChild();
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		Iterator<Tuple> rI = page.reverseIterator();
		Tuple tuple = null;
		for (int i = numTuples / 2; i < numTuples; ++i) {
			tuple = rI.next();
			tupleList.add(tuple);
		}
		...
		newLeaf.setRightSiblingId(rightSiblingId);
		...
		//update parentEntry
		if (tuple == null)
			throw new DbException("what is wrong with my split leaf?");
		Field midF = tuple.getField(keyField);
		BTreeEntry parentEntry = new BTreeEntry(midF, page.getId(), newLeaf.getId());
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midF);
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int numSlots = page.getNumEntries();
		ArrayList<BTreeEntry> slotList = new ArrayList<>();
		Iterator<BTreeEntry> rI = page.reverseIterator();
		for (int i = numSlots / 2; i < numSlots - 1; ++i)
			slotList.add(rI.next());
		for (BTreeEntry s : slotList){
			page.deleteKeyAndRightChild(s);
			newInternalPage.insertEntry(s);
		}

		BTreeEntry mid = rI.next();
		page.deleteKeyAndRightChild(mid);
		Field midF = mid.getKey();
		BTreeEntry parentEntry = new BTreeEntry(midF, page.getId(), newInternalPage.getId());
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		//System.out.println("here is steal leaf");
		int spare = (sibling.getNumTuples() - page.getNumTuples()) / 2;
		Iterator<Tuple> it = isRightSibling ? sibling.iterator() : sibling.reverseIterator();
		ArrayList<Tuple> spareList = new ArrayList<>();
		for (int i = 0; i < spare; ++i)
			spareList.add(it.next());
		for (Tuple t : spareList){
		...
		else key = page.iterator().next().getField(keyField);
		entry.setKey(key);
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		//System.out.println("here is steal inter left");
		int spare = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;

		Iterator<BTreeEntry> it = leftSibling.reverseIterator();
		LinkedList<BTreeEntry> spareList = new LinkedList<>();
		for (int i = 0; i < spare; ++i)
			spareList.add(it.next());

		BTreeEntry mid = spareList.pollLast();
		...
		page.insertEntry(new BTreeEntry(oldKey, leftChild, rightChild));
		leftSibling.deleteKeyAndRightChild(mid);
		for (BTreeEntry s : spareList){
			leftSibling.deleteKeyAndRightChild(s);
			page.insertEntry(s);
		}
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		//System.out.println("here is steal inter right");
		int spare = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
		...
		for (BTreeEntry s : spareList){
			rightSibling.deleteKeyAndLeftChild(s);
			page.insertEntry(s);
		}
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		//System.out.println("here is merge Leaf");
		Iterator<Tuple> it = rightPage.iterator();
		ArrayList<Tuple> moveList = new ArrayList<>();
		while (it.hasNext())
			moveList.add(it.next());
		for (Tuple t : moveList){
		...
		if (newRightId != null){
			BTreeLeafPage rightSibling = (BTreeLeafPage) getPage(tid, dirtypages, newRightId, Permissions.READ_WRITE);
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		ArrayList<BTreeEntry> moveList = new ArrayList<>();
		while (it.hasNext())
			moveList.add(it.next());
		...
		for (BTreeEntry s : moveList){
			rightPage.deleteKeyAndLeftChild(s);
			leftPage.insertEntry(s);
		}
